...
public class MotImageDimSettingsDialog
  ...
  public MotImageDimSettingsDialog(Context context, @Nullable ResultListener resultListener) {
    ...
    if (this.mMotImage != null) {
      Drawable motImageLogo = context.getResources().getDrawable(R.drawable.radio);
    ...
    switch (which) {
      ...
      case -1:
        this.mContext
            .getSharedPreferences(SettingsActivity.prefname_settings, 0)
            .edit()
            .putInt(SettingsActivity.pref_key_dim_percent, this.mDimValPercent)
            .apply();
        if (this.mListener != null) {
