...
public class DatabaseHelper {
    ...
    /* renamed from: a */
    private void m71a(DabSubChannelInfo subChannelInfo) {
    ...
    /* renamed from: a */
    private boolean m68a(List list, DabSubChannelInfo subChannelInfo) {
    ...
    public void m70a(ChannelInfo qVar) {
        ...
        getClass();
        C0162a.m9a(append.append("preset").append(" freq=").append(qVar.freq).append(" subid=").append(qVar.subChannelId).toString());
        SQLiteDatabase sQLiteDatabase = this.mDatabase;
        StringBuilder append2 = new StringBuilder().append("DELETE FROM ");
        getClass();
        sQLiteDatabase.execSQL(append2.append("preset").append(" WHERE freq=").append(qVar.freq).append(" AND subid=").append(qVar.subChannelId).toString());
        updateFavCount();
    }

    /* renamed from: a */
    public int getNewStationCount(List list) {
        int new_stations = 0;
        List c = getServiceSubchannelInfo(0);
        for (int i = 0; i < list.size(); i++) {
            DabSubChannelInfo subChannelInfo = (DabSubChannelInfo) list.get(i);
            if (!m68a(c, subChannelInfo)) {
    ...
    public int getStationCountByFilter(int filter) {
        ...
        switch (filter) {
            case STATIONLIST_FILTER_PTY: //1
                query.append(" where pty=");
                query.append(this.mCurrentStation);
                break;
            case STATIONLIST_FILTER_FAV: //2
                query.append(" where fav=1");
                break;
            case STATIONLIST_FILTER_FAV_AND_PTY: //3
                query.append(" where fav=1 AND pty=");
                query.append(this.mCurrentStation);
                break;
        }
        Cursor rawQuery = this.mDatabase.rawQuery(query.toString(), null);
    ...
    public void deleteNonFavs() {
        C0162a.m9a("delete favorites from SQL table service");
        SQLiteDatabase sQLiteDatabase = this.mDatabase;
        StringBuilder append = new StringBuilder().append("DELETE FROM ");
        getClass();
    ...
    public void updateFav(DabSubChannelInfo subChannelInfo) {
        synchronized (this) {
            if (this.mDatabase.isOpen()) {
                C0162a.m9a("updateFav " + subChannelInfo.mLabel);
                ContentValues contentValues = new ContentValues();
                ...
                contentValues.put("ps", Integer.valueOf(subChannelInfo.mPS));
                contentValues.put("fav", subChannelInfo.mIsFavorite ? "1" : "0");
                String where = "freq=" + subChannelInfo.mFreq + " AND sid=" + subChannelInfo.mSID + " AND subid=" + ((int) subChannelInfo.mSubChannelId) + " AND bitrate=" + subChannelInfo.mBitrate;
                SQLiteDatabase sQLiteDatabase = this.mDatabase;
                getClass();
    ...
    private void updateFavCount() {
        synchronized (this) {
            if (this.mDatabase.isOpen()) {
                ...
                StringBuilder append = new StringBuilder().append("SELECT * FROM ");
                getClass();
                Cursor rawQuery = sQLiteDatabase.rawQuery(append.append("service").append(" where fav=1").toString(), null);
    ...
    public ArrayList<DabSubChannelInfo> getFavorites() {
        ...
        synchronized (this) {
            StringBuilder append = new StringBuilder().append("SELECT * FROM ");
            getClass();
            String query = append.append("service").append(" where fav=1").toString();
