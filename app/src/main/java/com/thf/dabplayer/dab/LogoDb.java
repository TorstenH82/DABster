...
public class LogoDb {
    ...
    private SQLiteDatabase getDb() {
        if (this.mDb == null) {
            String dbPathFile = new StringBuilder(String.valueOf(this.mContext.getFilesDir().getAbsoluteFile())).toString() + "/logos.db";
            this.mDb = SQLiteDatabase.openOrCreateDatabase(dbPathFile, (SQLiteDatabase.CursorFactory) null);
        }
        return this.mDb;
    }
    ...
    public boolean updateOrInsertStationLogo(StationLogo stationLogo) {
        ...
        synchronized (this) {
            ...
            contentValues.put("path", stationLogo.mLogoPathFilename);
            contentValues.put("sid", Integer.valueOf(stationLogo.mStationServiceId));
            String where = "station='" + stationLogo.mStationNameNormalized + "' AND sid=" + stationLogo.mStationServiceId;
            int rowsUpdated = getDb().update("logos", contentValues, where, null);
            C0162a.m9a("logo db updated " + rowsUpdated + " rows for " + stationLogo.mStationNameNormalized + " sid " + stationLogo.mStationServiceId);
            if (rowsUpdated == 0) {
    ...
    public String getLogoFilenameForStationAndIssue(String name, int sid, @Nullable LookupIssue issue) {
        String normalized;
        String result = null;
        if (name != null && (normalized = StationLogo.getNormalizedStationName(name)) != null && !normalized.isEmpty()) {
            synchronized (this) {
                if (!getDb().isOpen()) {
                    openOrCreateDb();
                }
    ...
    private String getExactSid(String normalizedStationName, String realStationName, int serviceId) {
        ...
        String queryStrUser = queryStr + " AND path like '%" + SettingsStationLogoActivity.LOGO_PATH_USER + "%'";
    ...
    private String getExactMatch(String normalizedStationName, String realStationName, int serviceId) {
        String result = null;
    ...
    private String getSubstring(String normalizedStationName, String realStationName, int serviceId, @Nullable LookupIssue issue) {
        String result;
