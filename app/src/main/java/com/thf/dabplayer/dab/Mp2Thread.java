...
public class Mp2Thread extends Thread {
    ...
    private class SharedPrefListener implements SharedPreferences.OnSharedPreferenceChangeListener {
        ...
        public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
            if (key.equals(SettingsActivity.pref_key_declip)) {
                Mp2Thread.this.mIsClippedSampleDetectionEnabled = sharedPreferences.getBoolean(SettingsActivity.pref_key_declip, true);
            } else if (key.equals(SettingsActivity.pref_key_declip_notification)) {
                Mp2Thread.this.mIsClippedSampleNotificationEnabled = sharedPreferences.getBoolean(SettingsActivity.pref_key_declip_notification, true);
            } else if (key.equals(SettingsActivity.pref_key_audioLevel)) {
    ...
    public Mp2Thread(Context context, RingBuffer rVar) {
        ...
        this.f121b = rVar;
        this.f126g = context;
        SharedPreferences pref_settings = context.getSharedPreferences(SettingsActivity.prefname_settings, 0);
        this.mIsClippedSampleDetectionEnabled = pref_settings.getBoolean(SettingsActivity.pref_key_declip, true);
        this.mIsClippedSampleNotificationEnabled = pref_settings.getBoolean(SettingsActivity.pref_key_declip_notification, true);
        this.mPrefListener = new SharedPrefListener();
        pref_settings.registerOnSharedPreferenceChangeListener(this.mPrefListener);
    }
    ...
    private void m26a(int sampleRateInHz, int channels) {
        ...
        if (sampleRateInHz > 0) {
            ...
            SharedPreferences pref_settings = this.f126g.getSharedPreferences(SettingsActivity.prefname_settings, 0);
        ...
        while (!this.f120a) {
            ...
            if (z) {
                synchronized (this.f121b) {
                    if (this.f121b.getNumSamplesAvailable() >= 1024 && (a = this.f121b.readBuffer(bArr, 1024)) == 1024) {
            ...
            if (a2 < 0) {
                ...
                if (i + a2 < bArr2.length) {
                    for (int decoder_get_samplerate2 = 0; decoder_get_samplerate2 < a2; decoder_get_samplerate2++) {
    ...
    private void notifyClippedSamplesDetected() {
        ...
        if (this.mIsClippedSampleNotificationEnabled && (playerHandlerWeak = Player.getPlayerHandler()) != null && (playerHandler = playerHandlerWeak.get()) != null) {
    ...
    public void setAudioState(int audioState) {
        ...
        if (this.f124e != null) {
            if (this.mAudioState == 200 && audioState == 202) {
                float volume = pref_settings.getFloat(SettingsActivity.pref_key_audioLevel, 1.0f);
                float volumeDucked = pref_settings.getFloat(SettingsActivity.pref_key_audioLevelWhenDucked, 0.5f);
